...
function Group({ groupType, groupItems, marginTop = '' }) {
    return (
        <div className="group-container" style={{ marginTop: (marginTop + ' rem') }}>
            ...
            <ul className='group-list'>
                {groupItems.map((item, index) => {
                    return (
                        <a className={index == 1 ? 'active' : ''} href="#" key={index}><li>{item}</li></a>
                    )
                })}
            </ul>
        </div>
...
function Sidebar() {
    const group_names = ['Welcome!', 'Hello World', 'Noice', 'Gamers', 'College Days'];
    ...
    return (
        <div className='chat-user-list'>
            ...
            <Group groupType={'Groups'} groupItems={group_names} />
