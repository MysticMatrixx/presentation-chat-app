...
export default function ChatContent({ groupId }) {
    ...
    useEffect(() => {
        const unsub = onSnapshot(q, (docSnap) => {
            ...
            docSnap.docs.forEach((doc) => {
                doc.data().created_at && message.push({ ...doc.data(), id: doc.id, created_at: doc.data().created_at.toDate().toLocaleTimeString('hi-IN', { hour: '2-digit', minute: '2-digit' }), })
            })
            // console.log(message);
    ...
    function setMessages(item) {
        ...
        return (
            <div className={`chat-content  ${isCurrentUser ? 'sender' : 'reciever'}`} key={item.id}>
                ...
                <p className='message'>
                    {item.type == 'file' ?
                        <embed className='type-file' src={item.content} alt='file' />
                        :
                        (item.content).trim()
                    }
    ...
    async function uploadImg() {
        ...
        const time = Date.now()
        const locationRef = ref(storage, 'groups/' + groupId + '/' + time + '-' + currentFileToUpload.name)
        clearTextArea()

        try {
            await uploadBytes(locationRef, currentFileToUpload)
            const url = await getDownloadURL(locationRef)
            await createUserMessage(groupMessageRef, url.trim(), 'file')
        } catch (err) {
            console.log(err)
        }
    }
    ...
    async function handleAddMessage(e) {
        ...
        if (currentFileToUpload) return uploadImg();
        ...
        try {
            ...
            setCurrentText('')
            await createUserMessage(groupMessageRef, (content.value).trim(), 'text');
        } catch (err) {
            console.log(err)
        } finally {
            content.value = ''
        ...
        <div className='chat-content-container'>
            ...
            <form className='input-message-container' onSubmit={handleAddMessage}>
                <div className='btn-camera'>
                    ...
                    <input type='file' accept="image/png, image/jpeg, image/gif" onChange={changeTextArea} />
                </div>
                <textarea className='message-box' name="content" placeholder='Type your message here...' value={currentText} onChange={e => setCurrentText(e.target.value)} disabled={false} autoFocus={true}>
                </textarea>
                ...
                <button className='btn-clear' onClick={clearTextArea} hidden={!currentFileToUpload}>
